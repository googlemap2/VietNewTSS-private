...
def synthesize_clone(text: str, voice: str, ref_audio: str | None) -> str:
    stamp = datetime.utcnow().strftime("%Y%m%d_%H%M%S_%f")
    output_path = Path(config.get("output_dir", "outputs")) / f"clone_{stamp}.wav"
    return engine.synthesize_to_file(text=text, output_path=str(output_path), voice=voice, ref_audio=ref_audio)


def synthesize_batch(text_blob: str, voice: str) -> list[str]:
    ...
    return engine.synthesize_batch_to_files(rows, output_dir=str(out_dir), voice=voice, prefix="web_batch")
...
with gr.Blocks(title="yourtts") as demo:
    ...
    with gr.Tab("Single"):
        ...
        voice_input = gr.Dropdown(
            ...
            value=config.get("voice", available_voices[0] if available_voices else "default"),
        )
        run_single = gr.Button("Synthesize")
        single_audio = gr.Audio(type="filepath", label="Generated Audio")
        run_single.click(fn=synthesize, inputs=[text_input, voice_input], outputs=single_audio)
    ...
    with gr.Tab("Batch"):
        ...
        run_batch.click(fn=synthesize_batch, inputs=[batch_input, batch_voice], outputs=batch_files)
    ...
    with gr.Tab("Clone"):
        ...
        run_clone.click(fn=synthesize_clone, inputs=[clone_text, clone_voice, clone_ref], outputs=clone_audio)
